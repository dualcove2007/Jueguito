...
const tcOverlay = document.getElementById('touchControls');
...
function setupTouchButtons() {
    const btnMap = {
        tbLeft:  ['ArrowLeft', 'KeyA'],
        tbRight: ['ArrowRight', 'KeyD'],
        tbDown:  ['ArrowDown', 'KeyS'],
        tbJump:  ['ArrowUp', 'KeyW'],
        tbShoot: ['Space'],
        tbPause: ['KeyP'],
    };
    ...
    for (const [btnId, keyCodes] of Object.entries(btnMap)) {
        ...
        btn.addEventListener('touchstart', function(e) {
            ...
            for (const t of e.changedTouches) {
                activeTouches[t.identifier] = keyCodes;
                for (const kc of keyCodes) keys[kc] = true;
            }
        }, { passive: false });
        ...
        btn.addEventListener('mouseup', function(e) {
            e.preventDefault();
            for (const kc of keyCodes) keys[kc] = false;
        });
    ...
    function releaseTouchKeys(e) {
        for (const t of e.changedTouches) {
            const released = activeTouches[t.identifier];
            delete activeTouches[t.identifier];
            if (!released) continue;
            for (const kc of released) {
                let stillHeld = false;
                for (const other of Object.values(activeTouches)) {
                    if (other.includes(kc)) { stillHeld = true; break; }
                }
                if (!stillHeld) keys[kc] = false;
            }
        }
    }
}
    ...
    function onCanvasTap(e) {
        ...
        if (gameState === STATE.MENU) {
            ...
            for (let i = 0; i < options.length; i++) {
                const hx = GAME_W / 2 - hitW / 2;
                const hy = optionY[i] - hitH + 8;
                if (gx >= hx && gx <= hx + hitW && gy >= hy && gy <= hy + hitH) {
                    initAudio();
                    menuSelected = i;
                    if (i === 0) {
                        startGame();
...
function render() {
    ...
    if (isTouchDevice) {
        tcOverlay.style.display = (gameState === STATE.PLAYING) ? 'block' : 'none';
    }
}
